...
### Added
- **`/task:to-architecture` — the technical layer for a roadmap.** It writes an `## Architecture` section inside `.task/roadmap/<slug>.md`: components, the interfaces between items, a module-level sketch per item, and the technical ordering. Until now that shape had no home: items stay behavioral and specs pin individual decisions, so it leaked into specs or got lost. Fresh mode captures the roadmap first through the shared capture flow; enrich and revise work on an existing one, like `to-plan` does on a task. The ordering is mirrored into unchecked items' **Dependencies**, and missing `Spec:` headers are wired, because the driver's waves and the plan agents read only those. It stops on a fully checked roadmap, where no planner would read the section.
- **Planners follow the roadmap architecture** — `plan-driver.md` § Core step 1 reads the source roadmap's `## Architecture`, so interactive `to-plan` and the driver's plan agents share one rule. It is the intended shape, not a fixed anchor: a plan step may depart from it when the code disagrees, and says why in its Goal. The `to-plan` digest gains an architecture line; `to-task` keeps its Description behavioral.
- **`validate.sh` checks the architecture section** — a duplicated section, missing required sub-headings, a numbered sub-heading inside it, and `#N` references naming no item (a renumbering strands them silently). All of these are WARNs: no parser reads the section, and an ERROR would stop `roadmap-to-workflow` from launching over a stale sketch. The summary now also counts WARN lines emitted by the awk pass, which it used to drop.
- **`grill` routes technical-shape initiatives to `to-architecture`** — a new routing test sits between the spec and roadmap tests. The spec test is narrowed to lines naming a concrete artifact and a rejected alternative, or it would swallow every interface between items.
- **`to-spec` applies a decision test** — each section must name a concrete artifact and a rejected alternative, so a restated roadmap item no longer passes as a decision. Pure technical shape is routed to `to-architecture`, and anything the test drops is named in the digest instead of vanishing.
- **An eval case for `to-architecture`** covering fresh mode end to end. `evals/README.md` records that the older cases share three runner-format defects this one avoids, so a red result there is not read as a prompt regression.

### Changed
- **`.task/` ignores itself** — setup writes `.task/.gitignore` with the pattern `*`, covering the file itself, instead of adding a line to `.git/info/exclude`. The rule travels with the folder: `rm -rf .task` really leaves nothing behind, and a later `git init` still hides it. Setup no longer skips the step outside a git repository. The one cost: ripgrep applies the nested `*` to the folder's own files, so a search scoped to `.task/` comes back empty — the website has a troubleshooting entry for it.
- **`preflight.sh` restores a missing `.task/.gitignore`** — the skills promised a lost marker would come back, but no code did it, since setup runs only on first use. Preflight is the one path every skill except `grill` takes on an existing root. It never rewrites an existing file and never writes before the root is confirmed.
- **The roadmap capture flow has one owner, `skills/_lib/roadmap-capture.md`** — `to-roadmap` Steps 1–5 point into its Core, so `to-architecture`'s fresh mode captures a roadmap without a drifting copy. The flow is caller-aware: `to-roadmap` recommends the architecture follow-up, and the collision chip warns before an overwrite destroys an existing `## Architecture`.
- **The driver gates on an exact digest** — the implement and review stages must end with `OK #N <item-slug>`; anything else is a hard stop. The old `startsWith('FAIL')` check read a drifted line such as `**FAIL** #3 …` or a closing code fence as a pass, which let the mark stage tick an item whose review had failed. The check is the pure `digestPassed()`, pinned by `tests/driver-digest.test.sh`.

### Fixed
- **A copied repository used the original's artifacts** — after `cp -R`, the copy's `task.root` still named the original, which still held a `.task/CLAUDE.md`, so the copy read and wrote there and ignored its own `.task/`. `resolve-ws.sh` now accepts the anchor only when its git common dir is ours, or when it is exactly `dirname(git-common-dir)`, which keeps a bare repo's container resolvable. Tests cover copies, unrelated anchors, linked and bare worktrees, `--separate-git-dir`, and a submodule anchored at its superproject.
- **The driver's plan agent could fail on a rerun** — with no `TASKS:` list, `roadmap-item.md`'s ownership check could be skipped, so after a failed implement the agent called `write-task.sh --fresh` on the item's own file, hit exit 4 and failed instead of promoting or revising it. The check now keys on the file's existence.
- **`to-architecture` broke an empty Dependencies line** — appending to `**Dependencies:**` with no value wrote `**Dependencies:** , 3`, which `validate.sh` rejects. An empty value is now replaced, as `validate.sh` and `roadmap-items.sh` already treat it as no dependency.
- **`validate.sh` passed item number 0** — a `### - [ ] 0.` heading and a `0` in **Dependencies:** validated clean, yet the driver refused the launch with `bad args`. Both are now errors.
- **`validate.sh` dropped architecture WARNs on macOS** — in a UTF-8 locale macOS awk aborted on the `→` of an interface line. The awk checks now run under `LC_ALL=C`. An `## Architecture` heading with trailing text now counts as the section, and an `#N` right after a multibyte arrow (`#2→#9`) is no longer missed.
- **Every failure branch ends with a next-step footer** — `to-task` and `to-plan`'s write-failure branch and the driver's relayed `bad args — …` line had none.
- **Wording that contradicted the invariants** — collision warnings called `.task/` "git-excluded", which names the forbidden `info/exclude` mechanism; `to-architecture` Step 2 promised dependencies and titles for every item, while `roadmap-items.sh` prints them for unchecked items only.
- **Docs realigned with what ships** — the README and website reference pages show each command's real `argument-hint`; the contract credits roadmap parsing to `roadmap-items.sh` and gives `resolve-ws.sh` a Helpers row; the website documents `to-architecture` and the self-ignoring `.task/.gitignore`, and corrects the worktree `task.root` advice; `CONTRIBUTING.md` lists `roadmap-capture.md` and `.github/`.

### Removed
- **The repo-local `/self-improve` meta-skill** and its four lens agents — unused, and a duplicate of `/self-audit`'s steps. `/self-audit` itself now derives its rosters from disk instead of hardcoded copies that had drifted into false findings. Neither ships with the plugin.

## [3.6.0] — 2026-09-19

The deterministic half of every skill moves out of the prompts into bash helpers and the driver, the repo gains an executable test suite, and the autopilot driver is reached by its registered name — which is what makes it work anywhere but a dogfooding checkout. Non-breaking: no artifact-shape change, and existing `.task/` directories keep working untouched.

### Added
- **`skills/_lib/preflight.sh` prints the capture entry state in one call** — the resolved root, whether `.task/CLAUDE.md` exists, roadmaps with their progress and their unchecked item *numbers*, and the task / spec slugs, as a fixed parser-stable block injected into Step 0. It replaces the 2–4 bash round-trips each capture used to open with, and the listings that duplicated it: the roadmap picker's loop, the positional-slug resolve and the per-skill directory scans all read the block now. An absent config is reported as a line in the block rather than an exit code, so a skill branches on something it can already see. `validate.sh all` folds in under `VALIDATE:` for `roadmap-to-workflow`, the one caller that gates on the whole tree.
- **`skills/_lib/write-task.sh` is the single `task.md` writer** — the artifact template left three prompts for one script, so the header link forms, the separator, the section order and the stamped `## Execution` pointer have exactly one owner. It also performs the in-place surgery the model used to do by hand: promote inserts the plan above the pointer, revise replaces the plan and leaves an untouched `## Tests` byte-identical, and both repair a hand-edited target that lost its separator or pointer. Destructive cases are refused with distinct exit codes — an existing slug without `--force`, a target with no Description — so the caller's own collision chips stay in charge.
- **`skills/_lib/detect-project.sh` seeds first-run setup with facts** — setup's open-ended "analyze the project" becomes a printed block of what is actually there: manifests, the commands the project already declares, its commit-format doc, a documented TDD rule with its line. Prose language is reported as evidence (ASCII or not, plus a sample) and named by the model, which is the part a script should not decide.
- **`skills/_lib/roadmap-items.sh`** — the launcher's 45-line inline awk collector, lifted into a helper and covered by cases it could never have had inline. The terminator rules are kept verbatim, so it still agrees with `validate.sh` on what ends an item.
- **A bash test suite — `bash tests/run.sh`** — ten case files over root resolution, the roadmap tally and path resolver, `validate.sh`'s 0/1/2 exit contract, preflight, the task writer, project detection, the item collector, the driver's wave sort and its three-way registration seam, and the skill injections. Dependency-free (bash, awk, git; node for the driver-wave case, which skips without it), one process per case file, run on ubuntu and macOS from CI.
- **Plugin eval cases under `evals/`** — five cases covering what the bash suite cannot reach, since whether a skill fires at all, writes the artifact it promises and prints the interaction conventions is a property of the prompt and visible only in a real run. They are authored against the documented case format but unrun: `claude plugin eval` reports itself as early access and refuses. `evals/README.md` records that, and that `to-roadmap` has no case yet.
- **`argument-hint` on every skill**, mirroring its own **Input:** line and flag-free, so slash autocomplete shows the argument forms at the point of typing instead of leaving them buried in the body. The commands reference gained the argument column it was missing.

### Changed
- **The autopilot driver is invoked by its registered name, `task:roadmap-driver`** — declared in the plugin manifest under `"workflows"` — instead of by `scriptPath`. The Workflow tool permission-checks a `scriptPath` against the invoking session's working directory, which a plugin is never inside, so autopilot died before its first agent in every real install; it survived dogfooding only because there the plugin root *is* the cwd. A name that does not resolve hard-stops rather than sliding into the by-hand path: a stale or unreloaded plugin is an install defect, not a slow day. `tests/driver-registration.test.sh` now covers the manifest / `meta.name` / Step 2 seam, which a rename on either side used to expose only at someone's next autopilot run.
- **The driver computes the dependency waves itself** — the topological sort moved out of the prompt into `computeWaves(items, done, scope)`, and the args went from ready-made waves to items plus done plus scope. The skill reports what the roadmap says and passes the scope through, nothing more. A cycle or an out-of-scope dependency is caught before the first agent is spawned and returns one line naming the items, instead of a wave that runs work out of order.
- **`roadmap-to-workflow` hard-stops when the Workflow tool is absent.** The old "graceful fallback" had the skill loop the items itself, invoke `/task:to-plan` (`disable-model-invocation`, so silently skipped) and tick roadmap checkboxes by hand — a hand-rolled driver copy that broke the "orchestration stays with the platform" invariant. It now prints a by-hand recipe (`to-plan` in chat, implement in a fresh session, which ticks the box itself) and touches no file, the roadmap included.
- **The five capture / launcher skills state each rule once**, with the rationale moved into `docs/contract.md`: roughly 4,100 words out of the skill bodies, off every invocation's bill. Two pipelines gained a single owner in the process — `skills/_lib/plan-driver.md` § Core for the plan pipeline (`to-plan` Steps 3–7 are pointers into it, and the driver's plan agent follows it), and `skills/_lib/roadmap-item.md` for the from-roadmap block, read by `to-task`, `to-plan` and that same agent. What stayed in the skills is what needs a user: the testing-adjacent question and the pre-write collision chips.
- **`allowed-tools` grants one rule per helper script** a skill actually invokes, replacing a `_lib/*.sh` glob. A Bash rule matches raw command text, so the glob pre-approved any such script anywhere on disk, not just the plugin's own — the docs claimed otherwise and are corrected. The grant has to exist before the Step 0 injection that needs it, since a preprocessed command never prompts and one matching no pattern aborts the whole invocation.
- **`task:code-reviewer` reviews every implementation commit, not only `HEAD`** — it derives the diff base by walking back while each commit's files intersect the plan's **Touches**, bounded three ways (10 commits, a commit that plainly belongs to other work, and no walk at all without a Plan), and reports the range as its own mandatory output line. A plan landed one commit per step used to put every step but the last outside the review.
- **The reviewer's phase 2 sub-agents run on sonnet** — they read a file or subsystem and report claims, and that diff-wide fan-out is where most of the review's tokens go. Phase 3 stays on the reviewer's own model: proving or refuting a candidate is the judgement the pass exists for.

### Fixed
- **The declared workflow path must start with `./`** — the manifest schema requires it, so `"workflows": ["skills/_lib/roadmap-driver.js"]` failed validation with `workflows.0: Invalid input`, and a rejected manifest does not degrade to a plugin without a driver: it refuses to load the plugin at all, the six skills and the reviewer agent with it. Nothing in that error points at a missing prefix, so `CLAUDE.md` and the contract now say it outright.
- **A Step 0 fallback note aborted the skill before it started** — the note reproduced the `!`-injection syntax in all five bash-calling skills, and the platform's scan is positional, so the sentence itself ran as a second injection whose failure killed the invocation. Quoting does not save it: the pass that blanks ordinary code spans exempts a span preceded by a backtick or a `!`, which is exactly what double-backticking the example produces. `tests/skill-injections.test.sh` holds each of the five to exactly one injection, with the right kind argument, and `grill` to none.
- **`detect-project.sh` silently lost its language verdict** — a here-string feeding a `grep -q` that exits at the first match gets its command-substitution subshell killed with signal 11 on bash 5.x, printing `README_LANG:  — <sample>` with no verdict and not a word on stderr. Measured at 11–17 bad runs per 300 on 5.3.15; piping the sample instead gave 0 in 700, and macOS's own bash 3.2 never hit it. `setup.md` reads both language lines to pick the project's **Language**, and a blank verdict left it guessing from the sample.
- **Three helpers reported a success that had not happened** — `detect-project.sh` read a compact `"scripts": { … }` line as an empty block and harvested the *next* object's keys as script names, so setup could compose **Build and Tests** from a dependency name, in a file it writes once and never repairs; `write-task.sh` checked neither the mkdir nor the redirection nor the final `mv`, printing `WROTE:` and exiting 0 on an unwritable directory, where it now exits 5 with no success line; and promoting a plan into a hand-edited file that already carried `## Tests` anchored on the pointer and landed the plan below the tests, against the section order the contract fixes.
- **`preflight.sh`'s open-item collector accepted a heading no other parser does** — `### - [ ] 3.Title`, with no space after the dot, yielding `0/0 unchecked=3` and offering the pickers an item that `roadmap.sh`, `roadmap-items.sh`, `validate.sh` and the driver all ignore.
- **`validate.sh` compares duplicate item numbers numerically** — keyed on the raw digit string, `1.` and `01.` validated clean while the driver's mark awk and `validate.sh`'s own Dependencies check treat them as the same item and MARK-FAIL.
- **The test harness leaked its fixtures** — temp dirs were registered inside the command substitution callers capture the path with, so the cleanup trap always saw an empty array.
- **Docs realigned with what ships** — the reviewer commits on top and never folds its fixes into the implementation's commit; the repository structure lists the five new `_lib` helpers and both new top-level directories; the driver is passed items, done numbers and a scope rather than ready-made waves; the contract's pipeline diagram ends at the review stage like its two peers; `tests` and `evals` join the closed scope list; the website's guide and reference pages match the skills, with a first-run entry for a driver that does not resolve, quoting the error verbatim.
