...
- **`**Class:**` is a best-effort hint**, not a validated field. The skill infers a default from task shape at author time (Step 5 rubric), but the user may override it in-file. Empty / off-list values are tolerated by `validate.sh`; its one real consumer is the refine-phase Clarity auditor's missing-contracts check for substrate classes — leave it populated so that check can fire.
