...
- **Absent → inline setup.** Run it inline, do not defer to another command:
  ...
  3. Print the detected config as message text, then pose ONE `AskUserQuestion` confirmation (convention (b)) in the same reply — the chips don't display it:
     ```
...
5. Print the drafted Description body as message text, then pose an `AskUserQuestion` (Accept / Edit / Decline — same mechanism as chat-draft mode's Step 2.3) in the same reply, before writing anything. The chips don't render the draft, so the printed body is what the user judges. On **Decline**, write nothing and stop with "`task.md` not written. → Next: re-run `/task:to-task` when you want to capture it" (same closing line as chat-draft mode).
...
3. **Print the full draft as message text**, then — in the same reply — pose an `AskUserQuestion` (convention (b)) with chips **Accept** / **Edit** / **Decline**. The question box does not display the draft; skipping the printed text asks the user to accept something unseen:
