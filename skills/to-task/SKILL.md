...
3. **`CONFIG: present` → leave the file alone.** It is user-owned: never rewrite it, never re-detect its values, never "repair" a missing section. Nothing else is touched either: the `task.root` anchor and the `.git/info/exclude` line are written by first-run setup only and never re-checked here. Regenerating it is the user's move — delete, then re-run any capture.
