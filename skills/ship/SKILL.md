...
- **Single confirmation (interactive).** On every interactive ship, present the staged file list and the composed commit message **once**, then ask **exactly once** using the canonical **accept / decline / edit** grammar (convention (b)): **accept** — commit as shown and run the close; **decline** — abort without committing; **edit** — adjust the file list or message, then commit and close. The prompt always fires — there is no "if in doubt" conditional.
...
- End with the canonical next-step footer (convention (a)) — the task is closed and archived:
