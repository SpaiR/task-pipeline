...
5. Make `.task/` ignore itself: `[ -e "$AI_DIR/.gitignore" ] || [ -L "$AI_DIR/.gitignore" ] || printf '# task-pipeline: keeps .task/ out of git\n*\n' > "$AI_DIR/.gitignore"`. The `*` matches the `.gitignore` itself too, so nothing under `.task/` ever shows in `git status`, and the marker leaves with `rm -rf .task`. Run it outside a git repository as well — the file is inert there and takes effect after a later `git init`. Never touch the repository's `.git/info/exclude` or any tracked `.gitignore`; an existing `.task/.gitignore` is the user's and is left alone. `preflight.sh` recreates a missing one on every later run.
