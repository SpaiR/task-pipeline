...
- **Setup hard-stop.** Any skill except `to-task` / `to-plan` / `to-roadmap` / `to-spec` that does not check `.task/CLAUDE.md` and hard-stop when absent (via `require_config`). The four intake skills must instead auto-run the inline Step 0 setup in a fresh project rather than hard-stopping — writing the file without a confirmation chip, and never rewriting one that already exists.
