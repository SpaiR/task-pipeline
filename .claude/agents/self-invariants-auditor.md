...
- **Setup hard-stop.** Any skill except `to-task` / `to-plan` / `to-roadmap` / `to-architecture` / `to-spec` that does not check `.task/CLAUDE.md` and hard-stop when absent (via `require_config`). The five intake skills must instead auto-run the Step 0 setup (per `skills/_lib/setup.md`) in a fresh project rather than hard-stopping — writing the file without a confirmation chip, and never rewriting one that already exists.
...
- **Pipeline is invisible.** A skill making tracked edits outside `.task/` (`CLAUDE.md`, `README.md`, source files, a tracked `.gitignore`), or relying on a marker other than `git config task.root` + `.task/.gitignore` — including writing a `.git/info/exclude` entry (no active-task pointer, no `TASK_ID_OVERRIDE`, no per-worktree pointer file).
