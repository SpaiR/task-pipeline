...
|------|-------------|-------------|
| `.task-current` (worktree root) | `/task:design`'s open phase (initial mode); `/task:auto-roadmap`'s `auto-roadmap-design-runner` (via its `/task:design --from` initial path); `/task:go` (via inline open, both modes); removed by `/task:ship --full` | `_lib/resolve-ws.sh` (WS_DIR resolution); design's open phase `--from` continuation check; `/task:auto-roadmap` Step 0 and `/task:go --auto`'s `go-context.sh` preconditions; interactive `/task:go` (resume); `/task:ship --full` precondition |
...
| `.task/workspace/<task-id>/auto.lock` / `.task/workspace/<task-id>/auto-error.log` | `/task:auto-roadmap` (sentinel written in Substep 3.4 after `auto-roadmap-design-runner`'s `/task:design --from` lands `.task-current`; `orchestrator=auto-roadmap`) or `/task:go --auto` (Step 3.4; `orchestrator=go`, smaller field set); error log appended by the shared runners + `/task:auto-roadmap` orchestrator on FAIL via `_lib/fail-log.sh` | `/task:auto-roadmap` Step 0 gate 3 and `/task:go --auto`'s `go-context.sh` (both scan `workspace/*/auto.lock`); user (postmortem) |
