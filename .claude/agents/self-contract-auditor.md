...
- `skills/_lib/resolve-ws.sh` resolves `AI_DIR` via a path order that disagrees with `docs/contract.md` § "Root resolution" (`task.root` git config *on evidence* → ancestor walk for `.task/CLAUDE.md` *ceilinged at the top of this project* → `dirname(git-common-dir)/.task` → `$CLAUDE_PROJECT_DIR/.task` else `./.task`), or a consumer assumes a pointer or a `WS_DIR` — neither exists.
- `roadmap-to-workflow`'s driver contract diverges from `docs/contract.md` § "`roadmap-to-workflow` execution shape": the static `skills/_lib/roadmap-driver.js` invoked by its registered name `task:roadmap-driver` with `args` (never an inline-authored script, and never by `scriptPath` — the manifest's `"workflows"` key is what registers it), opus-plans (sonnet for `haiku`-hinted items)/sonnet-implements per item, plan agents reading `skills/_lib/plan-driver.md`, dependency-ordered waves, driver-side auto-mark via its serial mark stage (never the per-item plan/implement/review agents), stop-on-FAIL, digest last line `OK|FAIL #N <slug> <summary>`. A place where the SKILL.md or the driver script describes a different shape than the contract is a drift.
