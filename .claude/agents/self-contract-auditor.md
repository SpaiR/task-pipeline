...
- `skills/_lib/resolve-ws.sh` resolves `AI_DIR` via a path order that disagrees with `docs/contract.md` § "Root resolution" (`task.root` git config → ancestor walk for `.task/CLAUDE.md` → `dirname(git-common-dir)/.task` → `$CLAUDE_PROJECT_DIR/.task` else `./.task`), or a consumer assumes a pointer or a `WS_DIR` — neither exists.
