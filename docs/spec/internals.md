...
- **Executor-class — design half** (`auto-roadmap-design-runner.md`) — narrow-scope executor **shared by `/task:auto-roadmap` and `/task:go --auto`** (the `auto-roadmap-` name is historical). Produces `plan.md` (with the `Implement-Model:` stamp) on disk, keyed on its `from:` input: `<roadmap>#N` → open + blueprint for a roadmap item; `current` → blueprint only for a task the caller already opened. `tools:` and `model:` intentionally undeclared — inherits both from the parent interactive session.
- **Executor-class — build half** (`auto-roadmap-build-runner.md`) — narrow-scope executor **shared by `/task:auto-roadmap` and `/task:go --auto`**, spawned immediately after design-runner OK. Runs `/task:build` implement phase only for the task in `.task-current`, leaving the diff uncommitted for the caller's audit + ship. `tools:` intentionally undeclared (inherits parent toolset); `model:` intentionally undeclared in frontmatter — the caller sets it per spawn via `Agent.model` override, sourced from `plan.md → Implement-Model:` (`opus|sonnet|haiku`). This is the mechanism for the per-stage model split — implement can run cheaper than design + audit.
...
Executor scope is deliberately narrow — design-runner produces `plan.md` (open + blueprint, or blueprint-only when the caller pre-opened), build-runner covers implement, audit + ship stay in the caller's main thread. Widening either runner to swallow audit/ship would force audit's lens fanout onto the inline path (subagents can't spawn subagents), losing the parallel three-lens view.
...
- No `model:` — `auto-roadmap/SKILL.md`, `go/SKILL.md`, and reasoning-heavy orchestrators (`design`, `build`, `roadmap`); they inherit the parent session's model. For `auto-roadmap` (and `go --auto`) the main thread (design-runner spawn, audit lens fanout, ship) stays on the user's session model, while `auto-roadmap-build-runner` is spawned with a per-item `Agent.model` override read from `plan.md → Implement-Model:` (`opus|sonnet|haiku`) — see [auto-roadmap.md § Per-stage model split](auto-roadmap.md).
...
- **`workspace/<task-id>/auto.lock` sentinel schema.** Two producers, distinguished by the `orchestrator=` field: `/task:auto-roadmap` (Substep 3.4 — `roadmap`, `roadmap_mtime`, `start_item`, optional `items_filter`, `started`, `orchestrator=auto-roadmap`) and `/task:go --auto` (Step 3.4 — `orchestrator=go`, `started`, `working_dir`). Both write via `skills/_lib/auto-locks.sh write`. Spec: `docs/spec/artifact-contract.md`. Adding or removing a field touches: the producing SKILL's write call, the field-list in the spec, the field-list bullet in `README.md` artifact section, and any Substep reads.
- **Runner return-line contracts** — exact OK / FAIL strings for the two shared runners. The OK line is **orchestrator-agnostic**: parsers key on the `OK:` prefix + the trailing em-dash clause, never the log text in between (so a shared caller need not synthesize a roadmap `item #<N>`). Both must change together with **every** caller's parser (`auto-roadmap/SKILL.md` Substeps 3.3 / 3.6; `go/SKILL.md` Steps 3.5 / 3.7):
  - `auto-roadmap-design-runner.md` — OK: `OK: design done — plan.md ready, awaiting implement`. FAIL (post-open): `FAIL at <stage>: <reason>. Artefacts remain in .task/workspace/<task-id>/. See <error-log-path>.`. FAIL (pre-open, `from: <roadmap>#N` only): `FAIL at <stage>: <reason>. No workspace was created — nothing to clean up.`.
  - `auto-roadmap-build-runner.md` — OK: `OK: implement done — diff uncommitted, ready for audit`. FAIL (post-open only — workspace is guaranteed): `FAIL at implement: <reason>. Artefacts remain in .task/workspace/<task-id>/. See <error-log-path>.`.
