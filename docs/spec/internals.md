...
- `_lib/roadmap.sh` — shared roadmap utilities: `resolve_roadmap_path`, `roadmap_mtime`, `roadmap_progress_counts`. NOT auto-sourced from `preamble.sh` — most context scripts never touch roadmaps.
...
- `_lib/auto-locks.sh` — write-only helper for the orchestrator run lock `.task/roadmap/<slug>.lock`. Exposes `write_lock <path> kv1 kv2 ...` (atomic `set -o noclobber`); the driver never re-reads the lock, so no read primitive is exposed.
