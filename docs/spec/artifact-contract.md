...
- **`.task-current`** — per-worktree pointer. One line: the lowercase task-id of the active umbrella in this worktree. Written by `/task:design`'s open phase (initial mode) and (transitively, under `/task:auto-roadmap`, via the first item-runner's design-runner `/task:design --from`); removed by a default `/task:ship` full close (also via the `--full` alias). Gitignored through `.git/info/exclude` (configured by `/task:bootstrap`). Downstream skills resolve `WS_DIR=.task/workspace/<task-id>/` through this file via `skills/_lib/resolve-ws.sh` (priority order: `$TASK_ID_OVERRIDE` env > positional argument > `.task-current` contents). **Note:** neither the `/task:auto-roadmap` driver's own Step 2 nor the item-runner writes `.task-current` directly — the first item-runner's design-runner (`/task:design --from` initial-open path) does it. `.task-current` is **never** symlinked — it stays a real per-worktree file.
