...
- No parser consumes the section: item counting, `roadmap-items.sh` and the driver's mark stage all key on `### - [ ] N.` headings and pass it by, wherever it sits. `validate.sh` checks it advisory-only (see [§ validate.sh](#validatesh-optional-self-check-not-a-gate)).
...
| `resolve-ws.sh` | sourced, never run: the pure `.task/`-root finder that exports `AI_DIR`, sourced by every other helper here and by `validate.sh`. Order and evidence rules: [§ Root resolution](#root-resolution-skills_libresolve-wssh) and [§ resolve-ws.sh](#resolve-wssh-root-finder-only) |
