...
| `roadmap-capture.md` | **the roadmap capture flow**, one copy: the too-small precondition, context load, harvest-or-brainstorm branch, decision routing, drafting self-check, slug-collision guard, write, validate call and the report-only post-save self-check, plus the `## Forbidden` list binding every roadmap it writes. `to-roadmap` keeps only its Step 0 setup gate and its digest; its Steps 1–5 are a pointer into this file's `## Core` |
