...
`to-task` and `to-plan` add `Bash(bash *skills/_lib/write-task.sh* *)` — the helper that writes their artifact. Each skill grants the helpers it actually calls, and nothing else.
