...
- `roadmap-to-workflow` — the one launcher. Reports the roadmap's unchecked items and the user's scope, and invokes the plugin-shipped Workflow driver (`skills/_lib/roadmap-driver.js`, via `scriptPath` + `args`), which computes the dependency waves itself.
...
One line, identical in all five: the grant covers `skills/_lib/*.sh` (`preflight.sh`, `write-task.sh`, `detect-project.sh`, and the two that are sourced) plus `validate.sh`, and nothing outside the plugin's own directory. Naming the scripts one by one would mean re-editing five frontmatters every time a helper is added — this pattern's blast radius is already the plugin's helper directory.
