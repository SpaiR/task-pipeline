...
- `roadmap-to-workflow`'s auto-mark (ticking a roadmap item's checkbox) is a **driver stage** — a dedicated serial mark agent `skills/_lib/roadmap-driver.js` spawns right after that item's review returns OK — never part of the per-item plan/implement/review agents (avoids racing parallel writes to the roadmap file). The flip is **idempotent and self-reporting**: it keys on the item number over the full 5-state checkbox class, so an already-ticked item is a no-op reporting OK, and it echoes its outcome on stdout rather than leaving the agent to infer an invisible exit code. Exactly one heading for item N must exist; a missing, renumbered or duplicated heading stops the wave.
