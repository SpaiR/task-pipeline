...
- Interaction conventions, all three: (a) every user-facing output ends `→ Next: <command or artifact path>` or `→ Done.`; (b) every content-confirmation **prints the full draft as visible message text first**, then poses `AskUserQuestion` with **Accept** / **Edit** / **Decline** chips in the same reply — the question never carries the draft (never lean on the question box or an option's `preview`); Edit triggers a focused follow-up, then re-prints the draft as message text and re-shows; (c) every 2–4 option path fork uses `AskUserQuestion` chips. All flag-free — no `--plan`, `--from`, `--phase`, `--refine` anywhere user-facing.
