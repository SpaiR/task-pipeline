...
- **Executor-class** — three agents for `/task:auto-roadmap`: `auto-roadmap-item-runner.md` (spawned once per item by the driver loop; runs the whole per-item cycle in its own context) spawns `auto-roadmap-design-runner.md` (open + blueprint, parent-session model) and `auto-roadmap-build-runner.md` (implement only, per-spawn `Agent.model` from `plan.md → Implement-Model:`), runs audit adaptively (inline for trivial diffs, else fans out the three lens auditors itself), and runs ship inline. Design/build runners stay leaves (scoping choice — nested spawning is supported). Shared rules in `agents/_shared/runner-rules.md`. None declares `tools:` or `model:` in frontmatter.
...
- Design's blueprint stamps `Implement-Model: <opus|sonnet|haiku>` in `plan.md`; `validate.sh` enforces presence and value (position-agnostic). Load-bearing for the `auto-roadmap-item-runner`'s build-runner spawn. The rubric judges by reasoning difficulty (not diff volume) and, when `task.md` has a `Roadmap:` header, consults the source item's `**Class:**` as a **soft prior** (rote-refactor/content-vocabulary/tooling → lean haiku; product-feature/new-substrate/cross-module-migration → lean sonnet) — a missing/off-list `Class` is no signal, never a fail-stop.
...
- Per item: the driver spawns ONE `auto-roadmap-item-runner` (no `is_first`/`is_last` flags — every item is identical), then routes on its returned digest. The item-runner spawns `auto-roadmap-design-runner` → reads `plan.md → Implement-Model:` (fail-stop on miss/malformed) → spawns `auto-roadmap-build-runner` with `Agent.model: <value>` → runs audit **adaptively** (inline combined audit for a trivial diff, else fans out the three lens auditors itself) + ship inline (full close) → returns a compact report-card digest (last line parser-stable; carries `task_id:` and `roadmap_mtime:` on every OK). "Inline" = the item-runner reads each skill's `SKILL.md` (and phase file) and executes Steps directly. The driver never spawns runners/auditors or runs `/task:build` / `/task:ship` itself.
