...
- Every skill carries `disable-model-invocation: true` + `user-invocable: true`. (`validate` is a bash-only utility — `skills/validate/validate.sh` with no `SKILL.md` — so it has no frontmatter.) Artifacts and user dialog follow `config.md` → Language; parser-stable strings (headers, commit trailers, the `## Execution` block) stay English.
