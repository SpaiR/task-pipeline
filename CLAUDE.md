...
- `/task:bootstrap` Step 2 detects language policy + testing-policy mode from the repo (commit prose/docs, test infra) and presents both as a single proposal, confirmed with the accept/decline/edit grammar (`docs/spec/invariants.md § Interaction conventions`); edit is the override path for either value, decline writes no `config.md`. Detection only seeds the proposal — it never locks a value.
- `/task:design` is a thin orchestrator over 4 phase companions (`open`, `idea`, `blueprint`, `refine`). Refine is a repair-level phase off the everyday surface (not advertised in the frontmatter description, README signature, pipeline diagrams, or chain hints); it never auto-enters — only `--phase refine` or `--refine`. Documented once as repair-level in `docs/troubleshooting.md`.
