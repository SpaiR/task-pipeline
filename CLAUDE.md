...
- `WS_DIR` is always resolved through `skills/_lib/resolve-ws.sh` (priority `$TASK_ID_OVERRIDE` > positional > active-task pointer via `task_current_path`); bash helpers go through `_lib/preamble.sh` → `source_resolve_ws` (exceptions: `validate.sh` and `_lib/phase-detect.sh` source `resolve-ws.sh` directly). No helper may construct `.task/workspace/<file>.md` paths directly, and none hardcodes the pointer path — always `task_current_path`. `source_resolve_ws` runs `migrate_legacy_pointer` (moves a pre-upgrade worktree-root `.task-current` into the git dir) then self-heals a **provably-stale** pointer (empty, or its `workspace/<id>/` subfolder missing) via `heal_stale_pointer` — removing it with a one-line `note:` (not `ERROR:`) so resolution reports the clean "no active task" state; `resolve_ws` stays pure (reading the legacy worktree-root location as a read-only fallback), and the direct sourcers `validate.sh` / `phase-detect.sh` never migrate/heal (read-only detection). A pointer whose workspace exists is a valid in-flight task and is never cleared.
- `AI_DIR` (the `.task` root) is resolved by `find_ai_dir` in `resolve-ws.sh` — precedence `git config --local task.root` > upward walk for a `config.md` ancestor > `dirname(git-common-dir)` > `$CLAUDE_PROJECT_DIR/.task` > relative `.task`. The `task.root` anchor makes every worktree (nested, sibling, bare) resolve the same shared `.task/` with zero setup; the git-common-dir fallback covers pre-anchor repos. It exports an **absolute** `<root>/.task` with the `.task` component appended literally. Runs at `resolve-ws.sh` source time and is re-invoked (idempotent) by `preamble.sh`, `roadmap.sh`, and `validate.sh:require_config`. The active-task pointer is resolved through `task_current_path` (git per-worktree dir), never `dirname "$AI_DIR"`; the legacy worktree-root location is read only as a migration fallback. Only acts when `AI_DIR` is unset — never hardcode `AI_DIR=.task` ahead of it.
...
- `/task:design` is a thin orchestrator over 4 phase companions (`open`, `idea`, `blueprint`, `refine`). Its Step 0 config gate is intake-capable: on a fresh project it auto-runs `/task:bootstrap` inline then continues the original request (stops only on `decline`). Refine is a repair-level phase off the everyday surface (not advertised in the frontmatter description, README signature, pipeline diagrams, or chain hints); it never auto-enters — only `--phase refine` or `--refine`. Documented once as repair-level in `docs/troubleshooting.md`.
...
- Single per-umbrella sentinel `.task/workspace/<task-id>/auto.lock` (atomic `set -o noclobber`, written by the first item-runner's Step 2 after its design-runner lands the active-task pointer). Driver keeps run state in memory only (never reads the pointer / `auto.lock`). Step 0 gate 3 scans `workspace/*/auto.lock`. Clean finish → last item-runner ships a bare full close (slug from `summary.md`) which removes sentinel with workspace subfolder; failure → subfolder + sentinel retained as abort signal; user runs a bare `/task:ship` manually.
