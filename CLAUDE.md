...
- Every core command's user-facing output ends with the canonical next-step footer (`→ Next: <runnable command>`, or `→ Done.` when the flow is complete); every content-confirmation prompt uses the accept / decline / edit grammar; every discrete **path fork** (2–4 options) that can't be inferred is presented via `AskUserQuestion` chips — the structured-choice convention (c). All three defined once in [docs/spec/invariants.md § Interaction conventions](docs/spec/invariants.md#interaction-conventions-next-step-footer--choice-grammar); human-facing dialog only — parser-stable strings and artifact content untouched. (c) is **interactive-only**: flags (`--idea` / `--from` / `--items` / `--next` / `--auto`) stay as the non-interactive equivalent and skip the question; the `/task:auto-roadmap` runners never reach `AskUserQuestion` because the driver always passes explicit flags. Instances: `auto-roadmap` launch wizard (precedent), `/task:design` entry fork, `/task:build` implement→audit advance, `/task:design --from` item picker, `/task:auto-roadmap` item-scope question.
...
- `/task:design` is a thin orchestrator over 4 phase companions (`open`, `idea`, `blueprint`, `refine`). Its Step 0 config gate is intake-capable: on a fresh project it auto-runs `/task:bootstrap` inline then continues the original request (stops only on `decline` / `JOIN-REFUSE-*`). Refine is a repair-level phase off the everyday surface (not advertised in the frontmatter description, README signature, pipeline diagrams, or chain hints); it never auto-enters — only `--phase refine` or `--refine`. Documented once as repair-level in `docs/troubleshooting.md`.
...
- Design's Step 1 presents an **entry fork** (`AskUserQuestion`: brainstorm / draft directly / open from a roadmap — convention (c)) only on an ambiguous fresh start (auto-detect `open`, no positional context, no flag); prose → quick-draft, a flag → that flag wins, both skip the fork. Interactive-only: the `auto-roadmap-design-runner` always gets `--from`, so the fork is unreachable non-interactively (bare no-context defaults to idea/architect there). `--from` without `#<N>` and >1 open item → open.md item picker (convention (c); `#<N>` skips it).
- Build's manual-mode Step 5 asks an **implement→audit advance question** (`AskUserQuestion`: Run audit now / Stop here — convention (c)) after a clean interactive implement, reusing the `--auto` loop-back for one opt-in advance (not a full `--auto` run). Interactive-only + clean-only: never under the item-runner (drives literal flags) or after a quick-fix-exhausted hand-off. `--auto` stays the explicit non-interactive one-shot.
...
- Design's open refuses if `.task-current` exists, with two relaxations: from-roadmap continuation (4 conditions, see spec), and provably-stale self-heal (empty / missing workspace subfolder → removed with a one-line notice, proceed as initial open). Writes `.task-current` on initial open; untouched in continuation.
...
- A clean interactive build proposes ship: at each clean endpoint (phase-detect `done`, audit-clean, `--auto` `done`) it flows into ship's single accept/decline/edit confirmation (confirm-gated `simplify-pipeline-surface.spec.md §2`, interactive-only — decline holds and prints the manual footer; blocking paths never propose; item-runner byte-stable).
- `/task:ship` has two modes: full close / subtask transition. Interactive ship **infers** the mode from remaining pending work (`commit-context.sh` emits `verdict: transition|full-close`) and proposes it in the single Step 3 commit confirmation — the user accepts or flips it via *edit*; `--next` is an explicit transition override. The inference is **interactive-only**: under `/task:auto-roadmap` the item-runner keeps literal-flag selection (bare → full close, `--next` → transition) so `close.sh` mode strings and autopilot stay byte-stable. Slug is always auto-derived (no hand-supplied slug), kebab-case English regardless of language. Full close sweeps `workspace/<task-id>/` and removes `.task-current`; transition keeps `task.md` (Description body cleared). Removed forms (`--full`, a positional slug) fail loud with a "removed — use X" message.
...
- `/task:roadmap` Step 0 config gate is intake-capable (like design): on a fresh project it auto-runs `/task:bootstrap` inline then proceeds with the brainstorm/refine (stops only on `decline` / `JOIN-REFUSE-*`). roadmap is already outside the PreToolUse hook matcher, so no hook change was needed for it — only `design` had to be dropped from the matcher.
